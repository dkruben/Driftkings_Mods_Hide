package
{
	...
	public dynamic class NationTreeNodeSkinned extends NationTreeNode
	{
		...
		{
			super.validateData();
			var rawText:String = String(this.nameTF.text);
			var textParts:Array = rawText.split("||");
			// Python sends 7 elements: name||marks||posX||posY||height||width||mastery
			if(textParts.length == 7)
			{
				...
				// Parse with NaN validation and defaults
				var parsedX:int = int(textParts[2]);
				var parsedY:int = int(textParts[3]);
				var parsedH:int = int(textParts[4]);
				var parsedW:int = int(textParts[5]);
		
				this._markPosX = isNaN(parsedX) ? 75 : parsedX;
				this._markPosY = isNaN(parsedY) ? -12 : parsedY;
				this._markHeight = isNaN(parsedH) ? 24 : parsedH;
				this._markWidth = isNaN(parsedW) ? 100 : parsedW;
		
				this._cachedText = textParts[1];
				this._masteryCachedText = textParts[6];
			...
			{
				DebugUtils.LOG_ERROR("validateData: Array length is " + textParts.length + " should be 7!");
			}
			else
			{
		...
		internal function __setProp_button_NationTreeNodeSkinned_button_0(frameNumber:int) : *
		{
			if(button != null && frameNumber >= 1 && frameNumber <= 81 && (this.__setPropDict[button] == undefined || !(int(this.__setPropDict[button]) >= 1 && int(this.__setPropDict[button]) <= 81)))
			{
				this.__setPropDict[button] = frameNumber;
				try
				{
		...
		internal function __setProp_button_NationTreeNodeSkinned_button_81(frameNumber:int) : *
		{
			if(button != null && frameNumber >= 82 && frameNumber <= 161 && (this.__setPropDict[button] == undefined || !(int(this.__setPropDict[button]) >= 82 && int(this.__setPropDict[button]) <= 161)))
			{
				this.__setPropDict[button] = frameNumber;
				try
				{
		...
		internal function __setProp_button_NationTreeNodeSkinned_button_161(frameNumber:int) : *
		{
			if(button != null && frameNumber >= 162 && frameNumber <= 361 && (this.__setPropDict[button] == undefined || !(int(this.__setPropDict[button]) >= 162 && int(this.__setPropDict[button]) <= 361)))
			{
				this.__setPropDict[button] = frameNumber;
				try
				{
		...
		internal function __setProp_button_NationTreeNodeSkinned_button_361(frameNumber:int) : *
		{
			if(button != null && frameNumber >= 362 && frameNumber <= 481 && (this.__setPropDict[button] == undefined || !(int(this.__setPropDict[button]) >= 362 && int(this.__setPropDict[button]) <= 481)))
			{
				this.__setPropDict[button] = frameNumber;
				try
				{
		...
		internal function __setProp_handler(event:Object) : *
		{
			var currentFrameNumber:int = int(currentFrame);
			if(this.__lastFrameProp == currentFrameNumber)
			{
				return;
			}
			this.__lastFrameProp = currentFrameNumber;
			this.__setProp_button_NationTreeNodeSkinned_button_0(currentFrameNumber);
			this.__setProp_button_NationTreeNodeSkinned_button_81(currentFrameNumber);
			this.__setProp_button_NationTreeNodeSkinned_button_161(currentFrameNumber);
			this.__setProp_button_NationTreeNodeSkinned_button_361(currentFrameNumber);
		}
