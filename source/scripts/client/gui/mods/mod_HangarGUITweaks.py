...
def getCustomOutfit(self, vehicleCD, season):
    # from service
    ...
    if styleId != CUSTOM_STYLE_POOL_ID:
        return self.service.getEmptyOutfit()
    return self.service.itemsFactory.createOutfit(strCompactDescr=outfits.get(season, ''), vehicleCD=self._itemsCache.items.inventory.getItemData(vehicleCD).compDescr)
...
def handleResult(result):
    ...
    if result.success:
        return SystemMessages.pushMessage(text(
            R.strings.messenger.serviceChannelMessages.sysMsg.customization.remove()),
...
@override(ModuleContext, 'getStatsConfiguration')
def new_getStatsConfiguration(func, self, item, *args, **kwargs):
    value = func(self, item, *args, **kwargs)
    if g_config.data['enabled'] and g_config.data['showCompatibles']:
        value.showCompatibles = True
    return value


@override(_ParamsCache, 'getComponentVehiclesNames')
def new_getComponentVehiclesNames(func, self, typeCompactDescr, *args, **kwargs):
    if not (g_config.data['enabled'] and g_config.data['showCompatibles']):
        return func(self, typeCompactDescr, *args, **kwargs)
...
@override(CustomizationItemCMHandler, '_generateOptions')
def new_generateOptions(func, self, ctx=None, *args, **kwargs):
    ...
    if not getattr(_ctx, 'isPurchase', True) or self._item.isStyleOnly:
        return result
    appliedCount = 0
    # mostly stolen from tooltips code
    ...
    return result + [self._makeSeparator(), self._makeItem(
        REMOVE_FROM_OTHER, text(R.strings.vehicle_customization.propertySheet.actionBtn.clear()), optSubMenu=[self._makeItem(REMOVE_FROM_OTHER + '_' + str(vehicleCD), vehName) for vehicleCD, vehName in sorted(installedVehicles, key=lambda x: x[1])])]


@override(CustomizationItemCMHandler, 'onOptionSelect')
@process_waiting('customizationApply')
def new_onOptionSelect(func, self, optionId, *args, **kwargs):
    if not optionId.startswith(REMOVE_FROM_OTHER):
        func(self, optionId, *args, **kwargs)
        return
...
@override(OptDeviceItemContextMenu, '_generateOptions')
def new_generateOptions(func, self, ctx, *a, **k):
    result = func(self, ctx, *a, **k)
    ...
    if self._getVehicle().intCD in installedVehicles:
        return result
    return result + [self._makeSeparator(), self._makeItem(REMOVE_FROM_OTHER, text(R.strings.vehicle_customization.propertySheet.actionBtn.clear()), optSubMenu=[self._makeItem(REMOVE_FROM_OTHER + '_%s_%s' % (vehicle.intCD, vehicle.optDevices.installed.index(item)), vehName)for vehicle, vehName in sorted(installedVehicles, key=lambda x: x[1])])]


@override(OptDeviceItemContextMenu, 'onOptionSelect')
@adisp_process
def new_onOptionSelect(func, self, optionId, *a, **k):
    if not optionId.startswith(REMOVE_FROM_OTHER):
        func(self, optionId, *a, **k)
        return
    vehicleCD, slotID = map(int, optionId.split('_')[1:])
    yield ActionsFactory.asyncDoAction(ActionsFactory.getAction(ActionsFactory.REMOVE_OPT_DEVICE, self._itemsCache.items.getItemByCD(vehicleCD), self._getItem(), slotID, False))


@override(StorageCategoryPersonalReservesView, '__sort')
def new_sort(func, self, a, b, *_a, **k):
    if not (g_config.data['enabled'] and g_config.data['sort_personalReserves']):
        return func(self, a, b, *_a, **k)
    return int(a.count <= 1) or cmp(booster_tier(a), booster_tier(b)) or cmp(BOOSTERS_ORDERS[a.boosterType], BOOSTERS_ORDERS[b.boosterType]) or cmp(b.effectTime, a.effectTime)


@override(ProgressiveItemsView, '_getPossibleItemsForVehicle')
def new_getPossibleItemsForVehicle(_, self, *_args, **_kwargs):
    ...
    return [_item.compactDescr for _item in sorted((item for item in customizationCache.customizationWithProgression.itervalues() if item.itemType == CustomizationType.PROJECTION_DECAL and (item.filter is None or item.filter.matchVehicleType(vehicleType))), key=(lambda i: (next((j for j, tag in enumerate(decal_order) if tag in i.progression.levels[1]['conditions'][0]['path'][1]), 100), i.id)))]


@override(CarouselDataProvider, '_vehicleComparisonKey')
def new_vehicleComparisonKey(func, cls, vehicle, *args, **kwargs):
    if not (g_config.data['enabled'] and g_config.data['sort_vehicleCarousel'] and issubclass(cls, HangarCarouselDataProvider)):
        if getattr(func, '__func__', None) is None:
            # idiot-proof
            return func(vehicle, *args, **kwargs)
        return func.__func__(cls, vehicle, *args, **kwargs)
    return (
            not vehicle.isInInventory,
            vehicle.isOnlyForClanWarsBattles,
            not vehicle.isEvent,
            not vehicle.isOnlyForBattleRoyaleBattles,
            -vehicle.level, vehicle.isPremium,
...
@override(TankCarouselFilterPopover, '_generateMapping')
def new_generateMapping(func, _, *args, **kwargs):
    mapping = func(*args, **kwargs)
...
@override(TankCarouselFilterPopover, '_getInitialVO')
def new_getInitialVO(func, self, filters, *args, **kwargs):
    dataVO = func(self, filters, *args, **kwargs)
    if not g_config.data['enabled']:
        return dataVO
    if g_config.data['filter_vehicleCarousel_normal']:
        dataVO['specials'][2] = {
    ...
    if g_config.data['filter_vehicleCarousel_alliance']:
        for idx, alliance_orig in enumerate(reversed(nations.ALLIANCES_TAGS_ORDER)):
            ...
            dataVO['nations'][-(idx + 1)] = {
                ...
                'tooltip': makeTooltip(backport.text(R.strings.tooltips.blueprints.conversionView.title()).replace('%(allianceIcon)', icons.makeImageTag(backport.image(R.images.gui.maps.icons.blueprints.fragment.conversion.dyn(alliance)()))).replace('%(allianceName)', text_styles.stats(backport.text(R.strings.tooltips.blueprints.conversionView.alliance.title.name.dyn(alliance)()))), backport.text(R.strings.tooltips.blueprints.conversionView.subTitle()).replace('%(allianceNations)', text_styles.stats(backport.text(R.strings.tooltips.blueprints.conversionView.alliance.nations.dyn(alliance)())))),
...
@override(BasicCriteriesGroup, 'update')
def new_update(func, self, filters, *args, **kwargs):
    func(self, filters, *args, **kwargs)
    if not g_config.data['enabled']:
        return
    if g_config.data['filter_vehicleCarousel_normal']:
...
@override(Vehicle, 'isFullyElite')
def new_isFullyElite(func, self, *args, **kwargs):
    return func(self, *args, **kwargs) and (not self.postProgressionAvailability(True).result or self.postProgression.getCompletion() == PostProgressionCompletion.FULL)
