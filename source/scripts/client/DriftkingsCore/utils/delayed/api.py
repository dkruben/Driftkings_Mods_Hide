...
#    except ImportError:
#        print 'DriftkingsCore: ModsListApi package not found, ModsSettingsApi check skipped'
...
#            self._ContextMenuManager__currentHandler.api = SL.appLoader.getDefLobbyApp().containerManager.getViewByKey(ViewKey(VIEW_ALIAS)).api
...
#                'modsListApiIcon': '../mods/configs/%s/%s/%s.png' % (self.modsGroup, self.modSettingsID, self.modSettingsID),
    ...
    try:
        ...
        if mode == 'block':
            for blockID in config.blockIDs:
                vxSettingsApi.addMod(config.modSettingsID, config.ID + blockID, partial(config.createTemplate, blockID), config.getDataBlock(blockID), partial(config.onApplySettings, blockID), config.onButtonPress)
        else:
            vxSettingsApi.addMod(config.modSettingsID, config.ID, config.createTemplate, config.getData(), config.onApplySettings, config.onButtonPress)
    except ImportError:
        print config.LOG, '=' * 25
        print config.LOG, 'no-GUI mode activated'
        print config.LOG, '=' * 25
